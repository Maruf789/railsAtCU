...
2) I have created two partials-- "navbar" ("/layouts/_navbar.html.erb") and "footer" ("/layouts/_footer.html.erb"). 
    ...
    b) In the partial named 'footer', I have included an image of my App Logo, copyright info, and year. The path for the image is "/assets/images/logo.png" 

Sample Rundown:

1) First, I'm going to open "rails console" and create some User objects. The first two User objects are good instances, which means, they follow the validations (User.valid? "true"). The third instance will have incorrect field(s). And the fourth instance of User will have missing field(s) When I will try to save them, for the second and third instances the validation will be false, so rails will complain and display respective error messages. 
...
2) Once you run "mongod" and "rails s" in the terminal, you can opon "localhost:3000" and easily the footer visible there along with the "log in" header.  Now, you can go ahead and log in using the credentials of the first two User objects we created earlier, namely with the email/password combinations of either  "aim2122@columbia.edu" and "123"  OR "nc@mit.edu" and "abc". As soon as you log in, for each additional page you browse (the redirected pages based on the user actions/controller verbs), you'll notice the respective logically appropriate "navbar" [header] and the "footer" in each page.
